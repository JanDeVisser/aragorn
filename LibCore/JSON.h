...
public:
    ...
    template<>
    [[nodiscard]] std::optional<JSONValue> value() const
    {
        return *this;
    }
    ...
    template<std::floating_point Float>
    [[nodiscard]] std::optional<Float> value() const
    {
        switch (m_type) {
        case JSONType::Integer:
    ...
    [[nodiscard]] std::optional<B> value() const
    {
        switch (m_type) {
        case JSONType::Null:
            return false;
    ...
    template<>
    [[nodiscard]] DecodeResult<std::string> convert() const
    {
        return to_string();
    }
...
inline Error<JSONError> decode_value(JSONValue const &value, std::map<std::string, T> &target)
{
    ...
    for (auto const &[k, _] : map) {
        ...
        auto maybe = decode<T>(value, k, decoded);
        if (maybe.is_error()) {
            return maybe;
        }
        target[k] = decoded;
